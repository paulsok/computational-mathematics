...
x:=a;
n:=trunc((b-a)/h);
label3.Caption:='ѕри шаге h= '+floattostr(h)+' отрезок xИ['+floattostr(a)+';'+floattostr(b)+']'+
#13+' разбиваетс€ на '+floattostr(n)+' части(ей).';
while(x<=b) do
 begin
...
x:=a; // оп€ть присваиваем х значение 0 дл€ возможности использовать цикл с учловием.
g:=0.01; // задаем шаг построени€ точек. Ќапомню, что он должен быть много меньше шага h, иначе получитс€ не крива€, а ломана€.
while (x<=b) do
begin
 y:=1/sqr(x);
 x:=x+g;
