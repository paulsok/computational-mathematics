...
Procedure VK(G:fun;F:mass;var X:mass);
 Var
  C:mass;
  A,aa,bb:array[1..N] of real;  //вспомогательные коэфф.
...
k1:=strtofloat(edit2.text);//начальные условия
...
VK(B,F,C);  // метод правой прогонки.

For i:=0 to N do A[3,i]:=c[i];
  For i:=1 to N do
   begin
    A[4,i]:=(c[i]-c[i-1])/(6*R[i]);
    A[2,i]:=-R[i]*c[i]/2-sqr(R[i])*A[4,i]+(a[1,i]-a[1,i-1])/R[i]; //трёхдиагональнaя системa
   end;
  For i:=1 to 4 do         //значения вычисленных коэфициентов
for j:=1 to N do
stringgrid2.Cells[i,j]:=FloatToStrF(a[i,j],ffFixed,10,3); //вывод сплайна
...
If x0=x then label7.Caption:=floattostr(A[1,0]) else
  begin
    For i:=1 to N do
      begin
        x0:=x0+R[i];        //вычисление значения х0,значю ф-ции на промежутках
        If x<=x0 then
          begin
            j:=i;
            break;
          end;
      end;     //значения вычисленных коэфициентов подставляем в конечн формулу и
